...
$(document).ready(function() {
    ...
    function createTweetElement(tweetDataObject){
        ...
        const text = tweetDataObject.content.text;
        const timestamp = tweetDataObject.created_at;

        let $tweet = $('<article>').addClass('tweet');
        $tweet.append(`<header><img src="${img}"><h2>${name}</h2><span class='userID'>${handle}</span></header>`);
        $tweet.append(`<p>${text}</p>`);
        $tweet.append(`<footer>${timestamp}`);
        //$tweet.append(`<i class="fas fa-flag"></i>
        $tweet.append(`</footer></article>`);
        return $tweet;
    }
